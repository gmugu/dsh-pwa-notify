...
- **设置界面**：DSH 设置页「通知推送」卡片——推送状态（唯一的开启入口，已订阅置灰）、六类推送开关、对话摘要开关、按工作区静音、按当前配置发真通知的测试按钮（勾不勾摘要，测试和真实推送文案完全一致）、订阅设备管理。即时生效，持久化在 DSH 设置存储。
...
| 回合出错 / 目标受阻 / 后台任务结束 | 默认推（设置卡片可关；任务完成与失败是两个独立开关） |
| 回合结束 | 默认不推（设置卡片开启）；子代理永远不推 |

等授权 / 等回答不受防打扰间隔压制。通知全部由 Service Worker 的 `push` 事件展示，与页面状态无关。

**按工作区静音**（v0.8.4）：设置卡片勾选某个工作区即**完全静音**——该工作区的等授权、等回答、出错、受阻、任务结束、回合完成与 `notify_user` 调用都不再推送（「测试发送」按钮不受影响）。列表来自工作区注册表（侧边栏同一份数据），判定按会话工作目录与工作区的规范化路径匹配；无法归属工作区的事件（无 `cwd` 的会话、无归属的后台任务）不静音。

## 兼容性

兼容版本范围在 `package.json` 的 `peerDependencies` 里逐包声明（cordis + 本插件实际消费的 `@deepseek-ai/dsh-*` 运行时包），声明下限 **`^0.1.7-alpha.2`**，即 `>=0.1.7-alpha.2 <0.2.0`。已实测的宿主版本：

| @deepseek-ai/dsh | 状态 |
| --- | --- |
| 0.1.7-alpha.2 | ✅ 实测通过 |
| 0.1.5-alpha.2 ~ rc.2 | ⛔ 不再支持（0.9 起）。0.1.7 移除了 `settings.register`/`settingsScope` 与 `jobs.onJobDone`，本插件已迁到 Config schema（volatile 字段）+ `jobs.events.subscribe`，旧宿主上无法加载 |
| < 0.1.5-alpha.2 | ⛔ 未支持 |

DSH 升级后：若 `dsh web` 启动日志里本插件行报错、或设置卡片 / 推送异常，先对照新版 DSH 的 API 变化审计本插件（参考 `AGENTS.md` 的硬约束清单），同步修完后把 `peerDependencies` 与上表一起更新。
...
**开关**：设置页「通知推送」卡片，即时生效（通知文案为内置固定文案，`{question}/{summary}` 内容随「带对话摘要」开关变化）。「按工作区静音」存的是工作区路径数组（设置命名空间的 `mutedWorkspaces`）；没有工作区注册表时（如 Electron 组合）卡片显示空态，也可在设置文档 `dsh-pwa-notify` 命名空间里手写该数组（路径会做 `resolve` 归一后比较）。
