...
- **本地轮询兜底**：未订阅推送的浏览器（或 `push: false` 时）走轮询通道——页面后台运行时经 Service Worker 弹通知（后台 8s / 前台 40s）。订阅成功后推送接管展示，轮询**降为 5 分钟一次的心跳**：不再弹（不会重复响），只作推送静默失败时的安全网——FCM 端点不可达（大陆服务器直连 Google 被墙）、推送订阅过期等情况下，开着的页面最迟 5 分钟仍能收到「等你授权」，好过石沉大海；同时它支撑 `notify_user` 工具的「多少页面在听」计数。
